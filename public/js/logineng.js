        ...
        function processRecognitionResult(event) {
            ...
            if (awaitingRememberMeConfirmation) {
                if (spokenWord === "yes") {
                    ...
                    speakBack("Remember Me selected. Ready to log in? Say yes or no.");
                    awaitingLoginConfirmation = true;
                    awaitingRememberMeConfirmation = false;
                } else if (spokenWord === "no") {
                    document.getElementById("remember-me").checked = false;
                    speakBack("Remember Me not selected. Ready to log in? Say yes or no.");
                    awaitingLoginConfirmation = true;
                    awaitingRememberMeConfirmation = false;
                } else {
                    speakBack("Please say yes or no.");
            ...
            // Field switching
            if (spokenWord.includes("password")) {
                currentField = 'pin';
                speakBack("Switched to password field.");
                return;
            } else if (spokenWord.includes("user id")) {
                currentField = 'user-id';
                speakBack("Switched to User ID field.");
                return;
            }

            // Handle character deletion
            if (spokenWord.includes("delete")) {
                deleteLastCharacter();
                return;
            } else if (spokenWord === "done") {
                const userId = document.getElementById("user-id").value;
                const password = document.getElementById("pin").value;
                speakBack(`User ID: ${userId}. Password: ${password}. Remember me? Say yes or no.`);
                awaitingRememberMeConfirmation = true;
                return;
            }

            // Map letters, numbers, and symbols to spoken words
            const spokenToCharMap = {
                "a": "A", "b": "B", "c": "C", "d": "D", "e": "E", "f": "F", "g": "G", "h": "H", "i": "I", "j": "J",
                "k": "K", "l": "L", "m": "M", "n": "N", "o": "O", "p": "P", "q": "Q", "r": "R", "s": "S", "t": "T",
                "u": "U", "v": "V", "w": "W", "x": "X", "y": "Y", "z": "Z", "zero": "0", "one": "1", "two": "2",
